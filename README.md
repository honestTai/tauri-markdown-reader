...
- GUI 安装包内置 `md-reader.exe`，可被系统终端、Codex 或其他 Agent 调用。
- 中英文界面切换。
...
GUI 安装包会一起放入 `md-reader.exe`。也可以在本地构建后直接调用：
...
src-tauri/target/release/md-reader.exe search ./docs --query "关键词" --json
